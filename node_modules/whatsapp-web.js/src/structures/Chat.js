...
class Chat extends Base {
    ...
    async fetchMessages(searchOptions) {
        let messages = [];
        try {
            messages = await this.client.pupPage.evaluate(
                async (chatId, searchOptions) => {
                    ...
                    if (searchOptions && searchOptions.limit > 0) {
                        while (msgs.length < searchOptions.limit) {
                            try {
                                ...
                                if (!loadedMessages || !loadedMessages.length || loadedMessages.length === 0) break;
                    ...
                    for (const m of msgs) {
                        try {
                            // Defensive parsing catch block specifically targeting missing LID profiles
                            const model = window.WWebJS.getMessageModel(m);
                            if (model) results.push(model);
                        } catch (e) {
                            // Suppress individual missing LID model conversion errors to let other messages map safely
                        }
                    }
                    return results;
                },
        ...
        } catch (globalEvalError) {
            // Safe fallback if target closed or internal evaluation failed completely
            return [];
        }
